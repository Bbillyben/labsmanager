 ...
 *
 * This makes use of the 'labSave' and 'labLoad' functions
...
function loadTableFilters(tableKey) {
    ...
    var filterstring = labLoad(lookup, defaults);
...
function generateAvailableFilterList(tableKey) {
    ...
    html +=`<option value=''>Select filter</option>`; // `<option value=''>{% trans 'Select filter' %}</option>`;
...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    // Add download button
    if (options.download) {
        buttons += `<button id='download-${tableKey}' title='Download data' class='btn btn-outline-secondary filter-button'><span class='fas fa-download'></span></button>`;
    }

    buttons += `<button id='reload-${tableKey}' title='Reload data' class='btn btn-outline-secondary filter-button'><span class='fas fa-redo-alt'></span></button>`;

    // If there are filters defined for this table, add more buttons
    if (!jQuery.isEmptyObject(getAvailableTableFilters(tableKey))) {
        buttons += `<button id='${add}' title='Add new filter' class='btn btn-outline-secondary filter-button'><span class='fas fa-filter'></span></button>`;

        if (Object.keys(filters).length > 0) {
            buttons += `<button id='${clear}' title='Clear all filters' class='btn btn-outline-secondary filter-button'><span class='fas fa-backspace icon-red'></span></button>`;
        }
    }

...
def get_base_dir() -> Path:
    """Returns the base (top-level) LABSMANAGER directory."""
...
def get_setting(env_var=None, config_key=None, default_value=None, typecast=None):
    ...
    Arguments:
        env_var: Name of the environment variable e.g. 'LABSMANAGER_STATIC_ROOT'
...
def get_static_dir(create=True):
    ...
    sd = get_setting('LABSMANAGER_STATIC_ROOT', 'static_root')

    if not sd:
        raise FileNotFoundError('LABSMANAGER_STATIC_ROOT not specified')
...
def get_backup_dir(create=True):
    ...
    bd = get_setting('LABSMANAGER_BACKUP_DIR', 'backup_dir')

    if not bd:
        raise FileNotFoundError('LABSMANAGER_BACKUP_DIR not specified')

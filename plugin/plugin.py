...
class VersionMixin:
    ...
    def check_version(self, latest=None) -> bool:
        ...
        from labsmanager.version import labsVersionTuple

        latest = latest if latest else labsVersionTuple()
        min_v = labsVersionTuple(self.MIN_VERSION)
        max_v = labsVersionTuple(self.MAX_VERSION)

        return bool(min_v <= latest <= max_v)


class LabManagerPlugin(VersionMixin, MixinBase, MetaBase):
    """The InvenTreePlugin class is used to integrate with 3rd party software.

    DO NOT USE THIS DIRECTLY, USE plugin.LabManagerPlugin
    """

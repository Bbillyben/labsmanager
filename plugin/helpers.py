...
def get_entrypoints():
    """Returns list for entrypoints for labsmanager plugins."""
    # on python before 3.12, we need to use importlib_metadata
    if sys.version_info < (3, 12):
        return entry_points().get('labsmanager_plugins', [])
    return entry_points(group='labsmanager_plugins')
...
#     from labsmanager.ready import isInTestMode
...
def get_modules(pkg, path=None):
    ...
    for finder, name, _ in pkgutil.walk_packages(path):
        try:
            ...
            logger.debug(f" Error collecting modules {name} :{error}")
            # log to stack
